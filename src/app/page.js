    ...
    <div style={{ background: '#0D0D0D', minHeight: '100vh', fontFamily: "'DM Sans', system-ui, sans-serif", color: '#fff', overflowX: 'hidden' }}>
      ...
      <section style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '100px 24px 60px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 800, textAlign: 'center', position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ display: 'flex', gap: 14, justifyContent: 'center', flexWrap: 'wrap', animation: 'fadeUp .6s ease .3s both' }}>
            <button className="btn-primary" onClick={() => router.push('/auth/register')} style={{ fontSize: 16, padding: '18px 40px' }}>Créer mon restaurant gratuit →</button>
            <button className="btn-outline" onClick={() => router.push('/auth/login')}>Se connecter</button>
          </div>
          <div style={{ marginTop: 16, fontSize: 12, color: 'rgba(255,255,255,.35)', animation: 'fadeUp .6s ease .4s both' }}>Aucune carte de crédit requise • Gratuit pour commencer</div>
        </div>
      ...
      <section id="fonctionnalites" style={{ padding: '100px 24px', maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 60 }}>
          ...
          <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(32px, 5vw, 52px)', fontWeight: 800, lineHeight: 1.2, marginBottom: 16 }}>Tout ce dont votre<br />restaurant a besoin</h2>
          <p style={{ fontSize: 16, color: 'rgba(255,255,255,.5)', maxWidth: 480, margin: '0 auto' }}>Une solution complète pensée pour les réalités de la restauration en Côte d'Ivoire.</p>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16 }}>
          {[
            ...
            { icon: '🔔', title: 'Appel serveur', desc: "Le client appelle le serveur d'un simple bouton. L'alerte arrive instantanément sur le tableau de bord du gérant.", color: '#A855F7' },
            { icon: '📊', title: 'Rapports & CA', desc: "Suivez votre chiffre d'affaires jour par jour. Exportez vos données en CSV pour votre comptabilité.", color: '#EC4899' },
          ].map(f => (
      ...
      <section style={{ padding: '100px 24px', maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 56 }}>
          ...
          <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(28px, 4vw, 44px)', fontWeight: 800 }}>Ce qu'en disent nos restaurateurs</h2>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16 }}>
          {[
            { name: 'Kouassi Aya', role: 'Gérant, Maquis Chez Aya • Abidjan', text: 'Depuis MaquisApp, mes clients commandent eux-mêmes. Je passe moins de temps à courir entre les tables et plus de temps en cuisine.' },
            { name: 'Bamba Seydou', role: 'Propriétaire, Le Woro-Woro • Bouaké', text: 'Installation en 10 minutes, QR codes imprimés, et mes commandes arrivent directement sur mon téléphone. Vraiment bluffant.' },
            { name: 'Adjoua Marie', role: 'Gérante, Maquis La Détente • San-Pédro', text: 'Le suivi des commandes en temps réel a tout changé. On ne perd plus une seule commande et les clients sont contents.' },
          ].map(t => (
            <div key={t.name} className="feature-card">
              <div style={{ display: 'flex', gap: 2, marginBottom: 14 }}>
                {[1,2,3,4,5].map(i => <span key={i} style={{ color: '#FFB347', fontSize: 14 }}>★</span>)}
              </div>
      ...
      <section id="tarifs" style={{ padding: '100px 24px', maxWidth: 640, margin: '0 auto', textAlign: 'center' }}>
        <div style={{ marginBottom: 48 }}>
          <div className="tag" style={{ marginBottom: 16 }}>Tarifs</div>
        ...
        <div style={{ position: 'relative', background: 'rgba(255,255,255,.04)', border: '2px solid #FF6B35', borderRadius: 24, padding: '36px 32px', maxWidth: 420, margin: '0 auto' }}>
          ...
          <div style={{ textAlign: 'left', marginBottom: 28 }}>
            {[
              'Tables illimitées', 'Menu illimité', 'Commandes illimitées',
              'QR codes', 'Écran cuisine', 'Historique & rapports', 'Support WhatsApp'
            ].map(f => (
              <div key={f} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 0', borderBottom: '1px solid rgba(255,255,255,.06)' }}>
                <span style={{ color: '#00C851', fontSize: 15, flexShrink: 0 }}>✓</span>
                <span style={{ fontSize: 14, color: 'rgba(255,255,255,.8)' }}>{f}</span>
              </div>
            ))}
          </div>
          <div style={{ background: 'rgba(0,200,81,.1)', border: '1px solid rgba(0,200,81,.25)', borderRadius: 12, padding: '10px 14px', marginBottom: 20, fontSize: 13, color: '#00C851', fontWeight: 600 }}>
            14 jours gratuits • Sans carte bancaire
          </div>
          <button className="btn-primary" onClick={() => router.push('/auth/register')} style={{ width: '100%', fontSize: 15, padding: '16px' }}>
      ...
      <section style={{ padding: '80px 24px 120px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 640, margin: '0 auto' }}>
          ...
          <button className="btn-primary" onClick={() => router.push('/auth/register')} style={{ fontSize: 17, padding: '20px 48px' }}>Créer mon restaurant →</button>
          <div style={{ marginTop: 16, fontSize: 12, color: 'rgba(255,255,255,.3)' }}>Configuration en moins de 5 minutes</div>
        </div>
      </section>
      ...
      <a
        ...
        style={{ position: 'fixed', bottom: 24, right: 24, zIndex: 1000, width: 56, height: 56, borderRadius: '50%', background: '#25D366', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 20px rgba(37,211,102,.45)', animation: 'waPulse 2.5s ease-in-out infinite', textDecoration: 'none' }}>
